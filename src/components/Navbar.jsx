...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-floop-dark/95 backdrop-blur-md shadow-lg' : 'bg-transparent'
        }`}
    >
          ...
          {navLinks.map((link, i) => (
            <li key={link}>
              <a
                ...
                className={`font-nunito font-bold text-sm tracking-widest text-white hover:text-floop-pink transition-colors duration-200 ${i === 0 ? 'border-b-2 border-floop-gold pb-1' : ''
                  }`}
              >
                {link}
              </a>
